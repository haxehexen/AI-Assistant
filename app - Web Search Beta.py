...
GOOGLE_API_KEY = "AIzaSyDGTcSac3sKKGcYBYD2l_gBjW-ZAT8o5_g"   # "YOUR_GOOGLE_API_KEY"
GOOGLE_CX = "226959fadf2d04551"   # YOUR_SEARCH_ENGINE_ID"
...
# 1. CONFIGURATION
# ==========================
MODEL_PATH = r"D:\Tools\llama.cpp\models\meta-llama-3.1-8b-instruct-abliterated.Q4_K_M.gguf"
...
wiki = wikipediaapi.Wikipedia(
    language="en",
    user_agent="LocalAI-Assistant/1.0"
)
...
def needs_internet(query: str) -> bool:
    print("need internet")
    keywords = [
        "google", "wiki", "wikipedia"
    ]
    q = query.lower()
    return any(k in q for k in keywords)


def wiki_search(query: str, max_chars=1200) -> str:
    page = wiki.page(query)
    if not page.exists():
        return "", None

    text = page.summary[:max_chars]
    url = page.fullurl
    return text, url
...
def predict(user_input, current_file):
    ...
    runtime_context.append(
        new_message(
            "system",
            "DEBUG MODE: In your answer, first repeat exactly the first 200 characters "
            "of the provided reference text before answering the user."
        )
    )
    print("runtime_context: ", runtime_context)
    
    combined = trim_context(context + runtime_context)
